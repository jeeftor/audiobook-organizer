...
| W2 | Browser UI, ABS metadata-source organize | plain | Audiobooks | `make gui-test-abs` | Implemented. Playwright configures a real ABS connection and path mapping from Organize, selects Audiobookshelf metadata, inspects the non-mutating reviewed plan, runs the reviewed moves, and verifies the organized rendered result plus the required ABS scan-and-clean guidance against the Docker-backed fixture. |
