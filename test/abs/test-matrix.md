...
| M1B | `metadata.json` | metadata-enabled | Ebooks | `go test -tags=abs_e2e ./test/abs/e2e -run TestABSMatrixTODO_MetadataJSONBooksLifecycle -count=1 -v` | Stubbed and intentionally failing. Build after M1A/M1C lifecycle shape is stable. |
